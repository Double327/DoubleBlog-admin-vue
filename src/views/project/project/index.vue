<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <router-link to="/projectOperation/project/add">
          <el-button
              type="primary"
              icon="el-icon-plus"
              size="mini"
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="mini" icon="el-icon-edit">
            <router-link :to="'/projectOperation/project/edit/'+scope.row.id">编辑</router-link>
          </el-button>
          <el-popover
              placement="top"
              width="180"
...
export default {
  ...
  methods: {
    beforeInit() {
      this.base = '/project/project';
      this.modalName = '项目';
      return true;
    },
    handleSupportChange(row) {
      let text = row.support ? '推荐' : '取消推荐';
      this.$confirm('确定要' + text + row.title + '项目吗?', '警告', {
    ...
    handlePrivateChange(row) {
      ...
      this.$confirm('确定要将项目' + row.title + text + '吗?', '警告', {
        confirmButtonText: '确认',
        cancelButtonText: '取消'
      }).then(function () {
